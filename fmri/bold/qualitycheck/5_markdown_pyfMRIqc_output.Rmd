...
definitions <- c(
  ...
  "Using the method described above, this parameter shows the absolute number of volumes above the 0.3 motion limit.",
  
  "Using the method described above, this parameter shows the percentage of volumes above the 0.3 motion limit.",
  
  "Using the method described above, this parameter shows the absolute number of volumes above the 0.5 motion limit.",
  
  "Using the method described above, this parameter shows the percentage of volumes above the 0.5 motion limit.")
...
I have added the following parameter now:
...
To determine number/percentage of volumes affected by motion above a certain threshold, AFNI's *1d_tool.py -quick_censor_count* was used. This program uses the motion file created using *3dvolreg*, computes the temporal derivative of each vector (done as first backward difference): "Take the backward differences at each time point.  For each index > 0, value[index] = value[index] - value[index-1], and value[0] = 0", that is, it calculates the relative motion. Additionally, the columns (i.e. the 6 motion parameters) are then collapsed using "euclidean_norm" method (sqrt(sum squares)). This combines rotations (in degrees) with shifts (in mm) as if they had the same weight:
...
The Euclidean norm of the relative motion is then compared against the defined limits (i.e. 0.2, 0.3 and 0.5) and outliers (in volumes) are counted, i.e. if >= limit or <= limit, result is 1.
