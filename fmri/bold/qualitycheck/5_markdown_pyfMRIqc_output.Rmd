...
DV_plot <- c("Mean", "Mean_(mask)", "SD", "SD_(mask)", 
             ...
             "volumes", "threshold_02mm", "fraction_02mm", 
             "threshold_03mm", "fraction_03mm", 
             "threshold_05mm", "fraction_05mm",
             "threshold_1mm", "fraction_1mm") 

definitions <- c(
  "Mean voxel (all voxels) intensity over time using **all** voxels. The tool also provides a 2D png image (axial slices) as output for the mean voxel intensity. It can be used to see if there is any general signal loss.",
  
  "Mean voxel intensity over time using **voxels inside the brain mask** only.",
  ...
  "Standard deviation of voxel intensity over time using **voxels inside the brain mask** only.",
  
  "Mean values of voxel for SNR (I assume this is the mean voxel intensity over time of the voxels outside the brain mask and below SNR threshold, i.e. noise). ",
  
  "Standard deviation of voxel for SNR (I assume this is the tandard deviation of voxel intensity over time of the voxels outside the brain mask and below SNR threshold, i.e. noise).",
  ...
  "Using the method described above, this parameter shows the absolute number of volumes above the motion limit of 0.2. Framewise Displacement (sum of relative movements across all 6 parameters) with a threshold of 2mm has been used in MRIQC",
...
interpretation <- c(
  ...
  "When looking at the percentage of volumes above 0.2 motion, the average is 10% (though the median is lower and the mean is influenced by outliers). We see some strong outliers across tasks and groups (some up to 60%). Again, there is considerable variation across the experiment and subjects.",
  
  "As expected, the absolute numbers are lower for the 0.3 threshold compared to the previous one and the motion drops to half of what it was (even though the threshold was not doubled). The number of outliers also decreases, but the same extreme outliers can be identified.",
  
  "Mean drops to 5% when looking at the percentage of volumes above the 0.3 threshold. Additionally, this threshold seems to be less influenced by group and task.",
  
  "Again, the absolute number of volumes above the 0.5 threshold drops further in comparison to the previous threshold, but is still influenced by outliers in both groups and tasks and varies over the course of the experiment and across subjects.",
  
  "The percentage of volumes above 0.5 motion drops to around 2%, but there are subjects across both tasks and groups with up to 30% volumes affected.",
  
  "Only a small number of volumes have motion above the threshold of 1 and most subjects do not exceed this threshold at all. Those that do are labelled as outliers.",
  
  "On average, there are 0% of volumes affected by motion above 1mm, but some scans are affected by this with up to 10%.")
...
I have added the following parameters now to further investigate the motion issues:
...
threshold/fraction capture the absolute/relative **number of volumes** above a given threshold. pyfMRIqc, in comparison, captures the number of movements across all 6 motion parameters that exceed a given threshold. To determine number/percentage of volumes affected by motion above a certain threshold, AFNI's *1d_tool.py -quick_censor_count* was used. This program is akin to -censor_motion (""), but it only outputs the number of TRs that would be censored.
...
[Please note at this point that both pyfMRIqc & MRIQC assume a 50mm head radius in their translation from radians to mm.]  
  
The Euclidean norm of the relative motion is then compared against the defined limits (i.e. 0.2, 0.3, 0.5 and 1) and outliers (in volumes) are counted, i.e. if >= limit or <= limit, result is 1.
...
In total, there are (at least) the following subjects standing out in terms of their motion. I copied their Dropbox directory in case you would like to have a look at them yourself. 

* sub-control037 (https://www.dropbox.com/sh/t81d1uqsa97hhmc/AAAbfKwcz62rhZ7EPxhrT-SGa?dl=0)  
* sub-control045 (https://www.dropbox.com/sh/kkahfdy5fx8ariq/AAAln-EfJGbGOljzZC9RvSzYa?dl=0)  
* sub-experimental016 (https://www.dropbox.com/sh/cy1vld4t7avc63a/AAAoG-RROsgnLmkbbhVWUFlna?dl=0)  