...
foreach subj ($subjects)
    ...
    3dmask_tool -dilate_inputs 1 -inputs $output_dir/mask_anat."$subj"_task-"$task"+tlrc. -prefix $output_dir/mask_anat_dilated."$subj"_task-"$task"+tlrc.

    # remove eroded WM and eroded ventricle masks (anatomical follower, already in MI space & anatomical grid)
    3dcalc -a $output_dir/mask_anat_dilated."$subj"_task-"$task"+tlrc. -b $output_dir/follow_ROI_FSWMe+tlrc. -c $output_dir/follow_ROI_FSvent+tlrc. -expr 'a-b-c' \
        -prefix $derivoutdir/"$subj"_task-"$task"_space-MNI152NLin2009cAsym_res-epi_label-dilatedGM_mask.nii.gz
