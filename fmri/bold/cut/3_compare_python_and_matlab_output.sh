...
cut_root="$path""/derivatives/magictrickwatching/cut"
...
# create output file
printf "subject\tdiff_1_min\tdiff_1_max\tdiff_2_min\tdiff_2_max\tdiff_3_min\tdiff_3_max" > "$path""/derivatives/magictrickwatching/"matlab_python_diff.tsv #file header
...
for (( s=0; s<${num_subjects}; s++));
	...
	# define python file
	python_1="$cut_root"/"$subj_id"/"$subj_id"_task-magictrickwatching_run-1_desc-cut_bold.nii.gz
	python_2="$cut_root"/"$subj_id"/"$subj_id"_task-magictrickwatching_run-2_desc-cut_bold.nii.gz	
	python_3="$cut_root"/"$subj_id"/"$subj_id"_task-magictrickwatching_run-3_desc-cut_bold.nii.gz
	...
	printf "\n$subj_id\t$min_1\t$max_1\t$min_2\t$max_2\t$min_3\t$max_3" >> "$path""/derivatives/magictrickwatching/"matlab_python_diff.tsv
