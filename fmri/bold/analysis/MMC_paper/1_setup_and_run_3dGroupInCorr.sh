...
CS_dir=$anal_dir/rest/3dClustSim
...
cd $FC_dir
rm MNI* rh* lh*
3dcopy $template_path/$template $FC_dir/$template
...
# 3. create all 7 parcellations
#3dcalc -a AFNI_Yeo2011_7Networks_MNI152.nii.gz -expr 'amongst(a,1)' -short -prefix AFNI_Yeo2011_network1_visual.nii.gz
#3dcalc -a AFNI_Yeo2011_7Networks_MNI152.nii.gz -expr 'amongst(a,2)' -short -prefix AFNI_Yeo2011_network2_somatomotor.nii.gz
#3dcalc -a AFNI_Yeo2011_7Networks_MNI152.nii.gz -expr 'amongst(a,3)' -short -prefix AFNI_Yeo2011_network3_dorsalattention.nii.gz
#3dcalc -a AFNI_Yeo2011_7Networks_MNI152.nii.gz -expr 'amongst(a,4)' -short -prefix AFNI_Yeo2011_network4_ventralattention.nii.gz
#3dcalc -a AFNI_Yeo2011_7Networks_MNI152.nii.gz -expr 'amongst(a,5)' -short -prefix AFNI_Yeo2011_network5_limbic.nii.gz
#3dcalc -a AFNI_Yeo2011_7Networks_MNI152.nii.gz -expr 'amongst(a,6)' -short -prefix AFNI_Yeo2011_network6_frontoparietal.nii.gz
#3dcalc -a AFNI_Yeo2011_7Networks_MNI152.nii.gz -expr 'amongst(a,7)' -short -prefix AFNI_Yeo2011_network7_default.nii.gz
...
# set up FC for LL 
#3dSetupGroupInCorr -mask $code_dir/$epi_mask -prefix "$FC_dir"/$out_GroupInCorr -byte ./sub-*/func/*_task-"$task"_desc-fullpreproc_bold.nii.gz
...
#3dGroupInCorr -setA "$FC_dir"/$c_out_GroupInCorr.grpincorr.niml -verb -batch MASKAVE "$code_dir"/seeds_MMC
#3dGroupInCorr -setA "$FC_dir"/$e_out_GroupInCorr.grpincorr.niml -verb -batch MASKAVE "$code_dir"/seeds_MMC
...
for (( r=0; r<${num_seeds}; r++)); do
	...
	# clust alpha (0.05) = 5
    3dClusterize -inset corr_"${seeds[$r]}"+tlrc -ithr 1 -NN 1 -binary -2sided p=$p_cor -clust_nvox 5 -pref_map mask_pcor_"${seeds[$r]}".nii.gz -mask $MMC_dir/$gm_mask
    ...
    3dClusterize -inset out_"${seeds[$r]}"+tlrc -ithr 2 -NN 1 -binary -2sided -0.3 0.3 -clust_nvox 5 -pref_map Cluster_"${seeds[$r]}".nii.gz -mask $MMC_dir/$gm_mask
...
3dcopy Cluster_seedFC_pcc.nii.gz out_networks
...
# rename sub-bricks
3drefit -sublabel 0 "network1_visual" -sublabel 1 "network2_somatosensory" -sublabel 2 "network7_default" -sublabel 3 "sFC_vis" -sublabel 4 "sFC_aud" -sublabel 5 "sFC_pcc" out_networks+tlrc
