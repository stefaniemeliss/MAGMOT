...
CS_dir=$anal_dir/magictrickwatching/3dClustSim
ISC_dir=$MMC_dir/ISC
mkdir $ISC_dir
cd $ISC_dir
...
# copy ISC maps
#cp $ISC_path/sub*sub*/ISC_sub*sub*_task-magictrickwatching_z.nii.gz  $ISC_dir

# specify and run 3dISC command
#3dISC -prefix ISC_magictrickwatching -jobs 4                       \
...
# clust alpha (0.05) = 5
3dClusterize -inset ISC_magictrickwatching+tlrc -ithr 1 -NN 1 -binary -2sided p=$p_cor -clust_nvox 5 -pref_map mask_ISC_magictrickwatching.nii.gz -mask $MMC_dir/$gm_mask

# use mask to multiply it with output of sFC
3dcalc -a mask_ISC_magictrickwatching.nii.gz -b ISC_magictrickwatching+tlrc[0] -expr 'a*b' -prefix masked_ISC_"${seeds[$r]}".nii.gz
3dcalc -a mask_ISC_magictrickwatching.nii.gz -b ISC_magictrickwatching+tlrc[1] -expr 'a*b' -prefix masked_ISC_t_"${seeds[$r]}".nii.gz

# put them all together
3dcopy mask_ISC_magictrickwatching.nii.gz  out_ISC_magictrickwatching
3dbucket masked_ISC_t_"${seeds[$r]}".nii.gz -glueto out_ISC_magictrickwatching+tlrc
3dbucket masked_ISC_"${seeds[$r]}".nii.gz -glueto out_ISC_magictrickwatching+tlrc

# declare sub-brick to be t values
3drefit -substatpar 1 fitt  out_ISC_magictrickwatching+tlrc

# rename sub-bricks
3drefit -sublabel 0 "ISC" -sublabel 1 "ISC_t" -sublabel 2 "mask" out_ISC_magictrickwatching+tlrc
