...
output <- subset(output, output$ID != "exp41")
output <- subset(output, output$ID != "cont37")
output <- subset(output, output$ID != "exp40")
output <- subset(output, output$ID != "cont29")
index <- is.na(output$recognition == T)
output <- output[!index, ] # note cont40 has two items missing in recognition because the last two trials have not been displayed
index <- is.na(output$cuedRecallStrict == T)
output <- output[!index, ] # note exp has one trick with NA
...
outg <- outg + geom_bar(stat="identity", position="dodge") + geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width=.1, position=position_dodge(0.9))  + scale_x_discrete(limits=c("ext","int")) + labs(x="Experimental condition", y="Age", fill = "Experimental Condition", title = paste("demogs I", version_official)) + theme_classic() + scale_fill_discrete(guide=FALSE)
outg

# gender
output <- plyr::count(dfWide, c('gender','group'))
...
# Post20 (was Post19)	I was very relaxed in doing this experiment. ### (R) note: post19_score is already recoded!
# Post21 (was Post20)	I was anxious while working on this task.
# Post22 (was Post21)	I felt pressured while doing this task.

###experiment in general
# Post23 (was Post22)	I tried to find out how many people will be able to find the solution.
# Post24 (was Post23)	The amount of magic tricks presented was
# Post25 (was Post24)	There were no problems with the internet connection while I participated in the experiment.
# Post26 (was Post25)	I was able to see the magic tricks properly.
